...
We ran these experiments as part of a longer chain of experiments that get at more "interesting" research questions. That chain has led us to "scientific" results (that have been conditionally accepted as of now).

You might question inpublished (and, therefore non-peer reviewed) experiments. I urge you to use your judgment as to whether you trust them. Your judgment is probably just as good as the judgment of 3 random reviewers (who were probably reading quickly since they had a big stack of reviews to do). If it gives you comfort, these experiments were detailed in a paper submission. The paper was (justifiably) not accepted because the collection of 7 (!) experiments didn't tell a complete story. The reviews did commend the care in the individual experiments.
...
The **hypothesis**: viewers are *better* at estimating the proportion of a part (relative to the whole) using a pie chart than a bar chart. By "better" we mean both more accurate and faster. Though we were honestly unsure if we could measure both (for reasons I will explain later). In the actual experiment, we considered 4 chart types (but here, I will focus on Pie Chart and Bar Charts).
...
The details of the experiment do matter (We'll provide them elsewhere). Briefly, this was a crowd-sourced experiment run on Prolific in the web browser. Participants answered with a slider that had 5% increments, and the correct answers were always multiples of 5. But I hope that the "main effect" is big enough (and common sense enough) that you'll trust me.
...
Here is a subtly different experiment. Everything is basically the same, but we're going to change the charts very slightly by modifying the labels (or data in the table). We'll explicitly give the viewer the part-whole relationships. 

Now, there is no estimation involved. For the table you can read the answer. Everyone gets it exactly right. For the bar chart, you simply need to estimate the height of the bar on the scale. People are good at this. The story is quite different from estimating. Not surprisingly, people are really fast and accurate with tables and bar charts. 
